...
void URKinematics::forward(const double * q, double * T) 
{
    ...
    *T = (c6*(s1*s5 + ((c1*c234-s1*s234)*c5)/2.0 + ((c1*c234+s1*s234)*c5)/2.0) - (s6*((s1*c234+c1*s234) - (s1*c234-c1*s234)))/2.0); T++;
    *T = (-(c6*((s1*c234+c1*s234) - (s1*c234-c1*s234)))/2.0 - s6*(s1*s5 + ((c1*c234-s1*s234)*c5)/2.0 + ((c1*c234+s1*s234)*c5)/2.0)); T++;
    *T = ((d5*(s1*c234-c1*s234))/2.0 - (d5*(s1*c234+c1*s234))/2.0 - d4*s1 + (d6*(c1*c234-s1*s234)*s5)/2.0 + (d6*(c1*c234+s1*s234)*s5)/2.0 - a2*c1*c2 - d6*c5*s1 - a3*c1*c2*c3 + a3*c1*s2*s3); T++;
    *T = c1*c5 + ((s1*c234+c1*s234)*s5)/2.0 + ((s1*c234-c1*s234)*s5)/2.0; T++;
    *T = (c6*(((s1*c234+c1*s234)*c5)/2.0 - c1*s5 + ((s1*c234-c1*s234)*c5)/2.0) + s6*((c1*c234-s1*s234)/2.0 - (c1*c234+s1*s234)/2.0)); T++;
    *T = (c6*((c1*c234-s1*s234)/2.0 - (c1*c234+s1*s234)/2.0) - s6*(((s1*c234+c1*s234)*c5)/2.0 - c1*s5 + ((s1*c234-c1*s234)*c5)/2.0)); T++;
    *T = ((d5*(c1*c234-s1*s234))/2.0 - (d5*(c1*c234+s1*s234))/2.0 + d4*c1 + (d6*(s1*c234+c1*s234)*s5)/2.0 + (d6*(s1*c234-c1*s234)*s5)/2.0 + d6*c1*c5 - a2*c2*s1 - a3*c2*c3*s1 + a3*s1*s2*s3); T++;
    ...
    *T = (d1 + (d6*(c234*c5-s234*s5))/2.0 + a3*(s2*c3+c2*s3) + a2*s2 - (d6*(c234*c5+s234*s5))/2.0 - d5*c234); T++;
    *T = 0.0; T++; *T = 0.0; T++; *T = 0.0; T++; *T = 1.0;
}

void URKinematics::forward_all(const double * q, double * T1, double * T2, double * T3, double * T4, double * T5, double * T6) 
{
...
int URKinematics::inverse(const double * T, double * q_sols, double q6_des) 
{
    ...
    double q1[2];
    {
        ...
        {
            double div;
            if (fabs(fabs(d4) - fabs(B)) < ZERO_THRESH) div = -SIGN(d4)*SIGN(B);
            else div = -d4/B;
            ...
            if (arcsin < 0.0) q1[0] = arcsin + 2.0 * ANVIL_PI;
            else q1[0] = arcsin;

            q1[1] = ANVIL_PI - arcsin;
        }
        else if (fabs(B) < ZERO_THRESH) 
        {
            double div;
            if (fabs(fabs(d4) - fabs(A)) < ZERO_THRESH) div = SIGN(d4)*SIGN(A);
            else div = d4/A;

            double arccos = acos(div);

            q1[0] = arccos;
            q1[1] = 2.0 * ANVIL_PI - arccos;
        }
        else if (d4*d4 > R) 
        {
            return num_sols;
        ...
        else 
        {
            ...
            if (neg >= 0.0) q1[1] = neg;
            else q1[1] = 2.0 * ANVIL_PI + neg;

            if (pos >= 0.0)  q1[0] = pos;
            else q1[0] = 2.0 * ANVIL_PI + pos;
    ...
    double q5[2][2];
    {
        for(int i = 0; i < 2; i++) 
        {
            ...
            double div;
            if (fabs(fabs(numer) - fabs(d6)) < ZERO_THRESH) div = SIGN(numer) * SIGN(d6);
            else div = numer / d6;

            double arccos = acos(div);

            q5[i][0] = arccos;
            q5[i][1] = 2.0 * ANVIL_PI - arccos;
        }
    }
    
    {
        for(int i = 0; i < 2; i++) 
        {
            for(int j = 0; j < 2;j++) 
            {
                ...
                {
                    continue; // Fixme - no solution
                }

                double arccos = acos(c3);
                q3[0] = arccos;
                q3[1] = 2.0 * ANVIL_PI - arccos;
                ...
                for(int k = 0; k < 2; k++) 
                {
                    ...
                    else if (q2[k] < 0.0) q2[k] += 2.0 * ANVIL_PI;

                    if (fabs(q4[k]) < ZERO_THRESH) q4[k] = 0.0;
                    else if (q4[k] < 0.0) q4[k] += 2.0 * ANVIL_PI;
