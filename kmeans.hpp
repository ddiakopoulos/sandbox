                             ...
                             float collapseDistance)      // distance so small it is not worth bothering to create a new clump.
{
    ...
    {
        std::vector<float3> centroids(clumpCount, std::numeric_limits<float>::max());
        ...
        float3 old_error = std::numeric_limits<float>::max();   // old and initial error estimates are max float3
