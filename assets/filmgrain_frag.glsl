...
void main() 
{
	...
	vec2 rotCoordsR = coordRot(texCoord, time + rotOffset.x);
	vec3 noise = vec3(pnoise3D(vec3(rotCoordsR*vec2(width/grainsize,height/grainsize),0.0)));
  
	if (colored)
	{
		vec2 rotCoordsG = coordRot(texCoord, time + rotOffset.y);
		vec2 rotCoordsB = coordRot(texCoord, time + rotOffset.z);
